...
// 필수 입력이 덜 채워진 채로 '다음'을 눌렀을 때, 무엇이 비었는지 짚어주는 작은 팝업.
...
const Item = styled.li`
  ...
  padding: 12px 14px;
  border-radius: 10px;
  border: 1px solid ${({ $done }) => ($done ? '#cbd879' : 'rgba(217, 138, 119, 0.65)')};
  background: ${({ $done }) => ($done ? '#edf3d5' : '#fffbf1')};

  color: ${({ $done }) => ($done ? '#576b1a' : '#4a3a2f')};
  font-family: 'Noto Sans KR';
  font-size: 16px;
  font-weight: ${({ $done }) => ($done ? 500 : 700)};
`;
...
const Mark = styled.span`
  ...
  background: ${({ $done }) => ($done ? '#cbd879' : '#e6a794')};
  color: #fff;
  font-size: 13px;
  line-height: 1;
`;
    ...
    <PopupBackdrop onClick={onClose}>
      <PopupCard $center $gap={14} $padTop={36} onClick={(event) => event.stopPropagation()}>
        ...
        <PopupTitle $center $size={22}>
          {remaining}개만 더 채워주세요!
        </PopupTitle>
        <PopupSubtitle $center>아래 항목을 입력하면 다음으로 넘어갈 수 있어요</PopupSubtitle>

        <List>
          {items.map((item) => (
            <Item key={item.label} $done={item.done}>
              <Mark $done={item.done}>{item.done ? '✓' : '!'}</Mark>
              {item.label}
            </Item>
          ))}
        </List>
