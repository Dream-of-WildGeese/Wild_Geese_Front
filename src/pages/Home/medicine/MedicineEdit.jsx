...
function MedicineEdit() {
  ...
  const isAllDay = isEveryDay(days);
  const toggleAllDays = () => setDays(isAllDay ? [] : ALL_DAY_VALUES);
  ...
  if (loading) {
    return (
      <Page>
        <Content>
          <CloseButton onClick={() => navigate('/home/medicine')}>
      ...
      <Content>
        ...
        <ScrollArea>
          ...
          <InputGroup>
            ...
            <RepeatWrap>
              <RepeatChip type="button" $active={isAllDay} onClick={toggleAllDays}>
                매일
              </RepeatChip>
              {DAY_OPTIONS.map((day) => (
                <RepeatChip
